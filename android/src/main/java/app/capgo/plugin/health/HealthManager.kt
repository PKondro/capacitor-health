...
class HealthManager {
    ...
    suspend fun readSamples(
        ...
        when (dataType) {
            HealthDataType.STEPS ->
                    fetch(StepsRecord::class) { record ->
                        val payload =
                                createSamplePayload(
                                        dataType,
                                        record.startTime,
            ...
            HealthDataType.DISTANCE ->
                    fetch(DistanceRecord::class) { record ->
                        val payload =
                                createSamplePayload(
                                        dataType,
                                        record.startTime,
            ...
            HealthDataType.CALORIES ->
                    fetch(TotalCaloriesBurnedRecord::class) { record ->
                        val payload =
                                createSamplePayload(
                                        dataType,
                                        record.startTime,
            ...
            HealthDataType.CALORIES_ACTIVE ->
                    fetch(ActiveCaloriesBurnedRecord::class) { record ->
                        val payload =
                                createSamplePayload(
                                        dataType,
                                        record.startTime,
            ...
            HealthDataType.WEIGHT ->
                    fetch(WeightRecord::class) { record ->
                        val payload =
                                createSamplePayload(
                                        dataType,
                                        record.time,
            ...
            HealthDataType.HEART_RATE ->
                    fetch(HeartRateRecord::class) { record ->
            ...
            HealthDataType.SLEEP ->
                    fetch(SleepSessionRecord::class) { record ->
                        val duration =
            ...
            HealthDataType.SLEEP_STAGES ->
                    fetch(SleepSessionRecord::class) { record ->
                        record.stages.forEach { stage ->
                            val duration =
                                    Duration.between(stage.startTime, stage.endTime)
                                            .toMinutes()
                                            .toDouble()
                            val payload =
                                    createSamplePayload(
            ...
            HealthDataType.WORKOUT ->
                    fetch(ExerciseSessionRecord::class) { record ->
                        val duration =
                                Duration.between(record.startTime, record.endTime)
                                        .toMinutes()
                                        .toDouble()
            ...
            HealthDataType.HRV ->
                    fetch(HeartRateVariabilityRmssdRecord::class) { record ->
                        val payload =
                                createSamplePayload(
                                        dataType,
                                        record.time,
                                        record.time,
                                        record.heartRateVariabilityMillis.toDouble(),
                                        record.metadata
                                )
                        samples.add(record.time to payload)
                    }
            HealthDataType.RESTING_HEART_RATE ->
                    fetch(RestingHeartRateRecord::class) { record ->
                        val payload =
                                createSamplePayload(
                                        dataType,
                                        record.time,
    ...
    private suspend fun <T : Record> readRecords(
        ...
        do {
            ...
            response.records.forEach { record -> consumer(record) }
    ...
    suspend fun saveSample(
        ...
        val meta =
                Metadata(
                        clientRecordId = metadata?.get("clientRecordId"),
                        clientRecordVersion = metadata?.get("clientRecordVersion")?.toLongOrNull()
                                        ?: 0
                )
